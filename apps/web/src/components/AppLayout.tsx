...
export function AppLayout({ children }: { children: ReactNode }) {
  ...
  // Navigation structure matching ABOUT-EMS.md specifications
  const navLinks = [
    { 
      label: 'Dashboard', 
      href: '/dashboard', 
    ...
    { 
      label: 'My Assets', 
      href: '/my-assets', 
      roles: [ROLES.CLIENT, ROLES.TECHNICIAN] 
    },
  ];

  const userCanSee = (link: typeof navLinks[0]) => {
      return authState.user && link.roles.includes(authState.user.role);
  }
